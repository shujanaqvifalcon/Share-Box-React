...
import { useLocation, useRoutes,useNavigate } from "react-router";
    ...
    <>
      <div className="w-full relative ibox-parent pt-[60px] min-h-screen">
        ...
        {data?.map((i) => {
           ...
           <div className="text-[20px] font-[400]">{i.letter}{i.number}</div>
              <div className="flex items-center gap-5">
              <i
              onClick={()=>{viewBox(`${i.letter}${i.number}`)}}
              class="fas fa-eye opacity-90 cursor-pointer text-2xl"></i>
              <i
              onClick={()=>{
                handleDelete(i._id)
              }}
              class="fas fa-trash opacity-90 cursor-pointer text-2xl"></i>
              </div>
           </div>
              </>
          )})}
